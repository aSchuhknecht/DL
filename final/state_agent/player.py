...
# class JurgenClone(nn.Module):
#     def __init__(self, n_layer=2, n_hidden=16):
#         super().__init__()
#         self.n_layer = n_layer
#         self.n_hidden = n_hidden
#         L = [nn.Linear(11, n_hidden), nn.ReLU()]
#         for _ in range(n_layer):
#             L.append(nn.Linear(n_hidden, n_hidden))
#             L.append(nn.ReLU())
#         L.append(nn.Linear(n_hidden, 3))
...
class Team:
    ...
    def __init__(self):
        ...
        self.model = torch.jit.load(path.join(path.dirname(path.abspath(__file__)), 'jurgen_clone_85.pt'))
    ...
    def act(self, player_state, opponent_state, soccer_state):
        ...
        for player_id, pstate in enumerate(player_state):
            ...
            action = self.model(features[None])[0]
            acceleration = max(0, min(1, action[0].item()))
            steer = max(-1, min(1, action[1].item()))
            brake = int(action[2].item() > 0.5)
